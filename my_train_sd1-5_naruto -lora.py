...
def log_validation(pipeline, args, accelerator, epoch, is_final_validation=False):
    ...
    #         images.append(pipeline(args.validation_prompts, num_inference_steps=30, generator=generator).images[0])
    ...
    #         tracker.log({phase_name: [wandb.Image(image, caption=f"{i}: {args.validation_prompts}")
    #                                   for i, image in enumerate(images)]})
    ...
    for i in range(len(args.validation_prompts)):
        ...
        image.save(os.path.join(save_path, f"epoch_{epoch}_image{args.validation_prompts[i]}.png"))
...
def main():
    ...
    unet.add_adapter(unet_lora_config)      ### 这里是注入LoRA适配器的关键代码

    # # 函数只修改 unet，不返回值
    # def inject_lora_into_unet(unet, rank=4, target_modules=["q", "k", "v"]):
    #     """
    #     将 LoRA 层注入 UNet（原地修改）
    #     """
    #     def _recursive_replace(module):
    #         for name, child in module.named_children():
    #             if isinstance(child, nn.Linear) and any(t in name for t in target_modules):
    #                 lora_layer = LoRALayer(child, rank=rank)
    #                 setattr(module, name, lora_layer)
    #             else:
    #                 _recursive_replace(child)
    #     _recursive_replace(unet)
    #
    # inject_lora_into_unet(unet, rank=8)
    ...
    import tm_lora.patch as tm_lora
    tm_lora.apply_patch(unet, ratio=0.6, sx=3, sy=3)
    # print("使用了 ToMe")
    def unwrap_model(model):
        model = accelerator.unwrap_model(model)
        model = model._orig_mod if is_compiled_module(model) else model
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                train_loss = 0.0
                ## 每隔 `checkpointing_steps` 步保存一次模型
            ...
            time_list.append(timer() - start_time)
    ...
    record['Time'] = time_list
    record['lr'] = lr_list
    record['Loss'] = loss_list
    record = pd.DataFrame(record)
    import time
    record_name = time.strftime("%Y_%m_%d_%H.csv", time.localtime())
    record.to_csv(r'./%s' % (record_name), index=False)

    contain_txt = f"总时间：{sum(time_list):.2f}" + f"  平均时间：{sum(time_list) / len(time_list):.2f} \n"
